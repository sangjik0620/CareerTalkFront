...
export default function PaymentPage() {
  ...
  const [selectedProduct, setSelectedProduct] = useState(null); // 모달용
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-14">
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
            {products.map((product, idx) => (
              <ProductCard
                ...
                product={product}
                onSelect={handleOpenModal}   // ← 바로 결제 X, 모달 오픈
                highlight={idx === 1}
              />
            ))}
          </div>
        ...
        {/* 하단 안내 */}
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4 text-xs"
          style={{ color: "#9BA8C8" }}>
...
// ════════════════════════════════════════════════════════════
// CompareModal  —  핵심 컴포넌트
// ════════════════════════════════════════════════════════════
function CompareModal({ product, quota, paying, onConfirm, onClose }) {
  // 구매 후 예상 잔여량 계산
  const after = quota
    ? {
    ...
    >
      ...
      >
        ...
        <div className="px-7 pb-7 flex flex-col gap-3">
          ...
          {/* 보안 안내 */}
          <p className="text-center text-xs flex items-center justify-center gap-1.5 mt-1"
            style={{ color: "#C2CADF" }}>
...
// ════════════════════════════════════════════════════════════
// CompareRow  —  Before → After 한 줄 비교
// ════════════════════════════════════════════════════════════
    ...
    >
      ...
      <div className="flex flex-col items-center gap-0.5 px-1">
        ...
        <svg viewBox="0 0 20 10" className="w-6" fill="none">
          <path d="M0 5h16M12 1l4 4-4 4" stroke="#4F6EF7" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </div>
    ...
    >
      {highlight && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <span className="inline-flex items-center gap-1 px-4 py-1 rounded-full text-xs font-bold shadow-md"
      ...
      <div className="mb-5 mt-2">
        <h3 className="text-xl font-extrabold mb-1.5"
          style={{ color: highlight ? "white" : "#1a1d3a" }}>
          {product.productName}
        </h3>
      ...
      <ul className="space-y-2.5 mb-6">
        <FeatureItem icon={<svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z"/></svg>}
          label={`AI 이력서 분석 ${product.analysisCreditCount}회`} highlight={highlight} />
        <FeatureItem icon={<svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M8.25 4.5a3.75 3.75 0 117.5 0v8.25a3.75 3.75 0 11-7.5 0V4.5z"/><path d="M6 10.5a.75.75 0 01.75.75v1.5a5.25 5.25 0 1010.5 0v-1.5a.75.75 0 011.5 0v1.5a6.751 6.751 0 01-6 6.709v2.291h3a.75.75 0 010 1.5h-7.5a.75.75 0 010-1.5h3v-2.291a6.751 6.751 0 01-6-6.709v-1.5A.75.75 0 016 10.5z"/></svg>}
          label={`모의면접 ${product.mockCreditCount}회`} highlight={highlight} />
      </ul>

      <div className="mb-5">
        <span className="text-[13px] font-medium" style={{ color: highlight ? "rgba(255,255,255,0.6)" : "#9BA8C8" }}>
          총 금액
        </span>
        <div className="flex items-end gap-1 mt-0.5">
          ...
          <span className="text-base font-semibold mb-0.5" style={{ color: highlight ? "rgba(255,255,255,0.6)" : "#9BA8C8" }}>원</span>
        </div>
      </div>
